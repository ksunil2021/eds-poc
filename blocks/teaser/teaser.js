...
export default function decorate(block) {
  ...
  const handleError = (el, fallback) => el.addEventListener('error', () => {
    el.remove();
    fallback?.();
  });
  ...
  const renderVideo = () => {
    ...
    video.addEventListener('loadeddata', () => video.play().catch(() => (video.controls = true)));
  ...
  const isAlignedVariation = block.classList.contains('media-left-aligned')
